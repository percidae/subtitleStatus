...
my_talks = Talk.objects.filter(next_amara_activity_check__lte = start)
...
my_talks = Talk.objects.filter(needs_complete_amara_update = True)
...
my_talks = Talk.objects.filter(amara_complete_update_last_checked__lte = before_24h)
